...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      // ---- LOCAL PATCH (Naqla) -------------------------------------------
      // Upstream: zIndex = (i === ci) ? 120 : 100 + op*10 — the CURRENT segment is
      // always on top at opacity 1. Because every scene is a full-viewport
      // object-fit:cover layer, that fully hides the neighbour, so segments hard-SWAP
      // at the boundary and `crossfade` only ramps an already-hidden layer. Invisible
      // when seams are frame-identical (upstream's premise), but a visible cut when a
      // clip wasn't frame-locked to its predecessor.
      // Fix: order strictly by segment index, so the incoming scene fades in ON TOP of
      // the outgoing one and `crossfade` becomes a real dissolve. Symmetric when
      // scrolling back up, and still invisible on frame-identical seams.
